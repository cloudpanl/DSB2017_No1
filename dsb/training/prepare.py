...
def full_prep(step1=True,step2 = True):
    ...
    if not os.path.exists(finished_flag):
        ...
        pool = Pool(4) # TODO: increase the pool size for multiprocessing.
...
def preprocess_luna():
    ...
    if not os.path.exists(finished_flag):
        ...
        pool = Pool(4) # TODO: Increase pool size for multiprocessing.
...
def prepare_luna():
    ...
    # A mapping of luna file names to shorter ones.
    # e.g. '012' <- '1.3.6.1.4.1.14519.5.2.1.6279.6001.104562737760173137525888934217'
    luna_abbr = config['luna_abbr']
    # tmp folder to move renamed .raw files.
    luna_data = config['luna_data']
    # Locations of luna .mhd files.
    luna_segment = config['luna_segment']
    ...
    if not os.path.exists(finished_flag):
        ...
        luna_abbr_csv = os.path.join(os.path.dirname(os.path.realpath(__file__)), config['luna_abbr'])
