...
@sc.param(Int(key="numTrees", default=20))
@sc.param(String(key="featureSubsetStrategy", default="auto"))
@sc.param(Int(key="seed"))
@sc.param(Float(key="subsamplingRate", default=1.0))
def SPRandomForestClassifier(context):
    args = context.args

    formula = RFormula(
        formula="{0} ~ {1}".format(args.labelColumn, "+".join(args.selectColumns))
    )
    classifier = RandomForestClassifier(**args.params)
...
if __name__ == "__main__":
    SPRandomForestClassifier()  # pylint: disable=no-value-for-parameter
