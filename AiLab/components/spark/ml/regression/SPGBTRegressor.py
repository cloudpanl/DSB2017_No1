...
@sc.param(String(key="lossType", default="squared"))
...
def SPGBTRegressor(context):
    args = context.args

    formula = RFormula(
        formula="{0} ~ {1}".format(args.labelColumn, "+".join(args.selectColumns))
    )
    classifier = GBTRegressor(**args.params)
...
if __name__ == "__main__":
    SPGBTRegressor()  # pylint: disable=no-value-for-parameter
